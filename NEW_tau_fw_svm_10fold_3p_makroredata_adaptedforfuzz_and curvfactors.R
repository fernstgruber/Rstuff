...
fuzz10m <- preds[grepl("10m",preds)]
fuzz250m <- preds[grepl("250m",preds)]
preds<- preds[!(preds %in% c(fuzz10m,fuzz250m))]
predset=preds
#save(predset,file="fuzzymakropredset.RData")

tt=1:3 #number of best parameters in combination
mydir=paste("tau_svm_fw_10fold_5p_fuzz_",dependent,"_",predset_name,sep="")
dir.create(mydir)
#############################################################################################################################
#############################################################################################################################
k=1
for(k in 1:10){
  ...
  for(t in tt){
    ...
    for(g in predset_new){
      ...
      CM <- table(preds,modeldata[[dependent]])
      sumtau = tau(CM)
      predictions_metrics[predictions_metrics$index== eval(g),"tau"] <- 1-sumtau$tau
      endtime <- proc.time()
      time <- endtime - starttime
      print(paste(g, " with tau error.rate of ",1-sumtau$tau, "and time =",time[3]))
      #######################################################################
    }
    predictions_metrics <- predictions_metrics[order(predictions_metrics$tau),]
    
    minindex <- predictions_metrics[predictions_metrics$tau == min(predictions_metrics$tau),"index"][1]
    print(paste("###########################################################################################################################################################remove the metric ",minindex,"##############################################################################################################################################################",sep=""))
    result_df[t,"metric"] <- minindex
    result_df[t,"tau"] <- predictions_metrics[predictions_metrics$index == minindex,"tau"]
    ...
    CM <- table(preds,ktestdata[[dependent]])
    sumtau = tau(CM)
    result_df[t,"geheimertauerror"] <-  1-sumtau$tau
